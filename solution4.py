...
def lineToList(line: str) -> list[int]:
...
inclusionsCount: int = 0
for line in lines:
    # using a single letter variable because I'm about to repeat it a lot
    r: list[int] = lineToList(line)
    # one line!
    inclusionsCount += ( (r[0] - r[2]) * (r[1] - r[3]) <= 0 )

print(inclusionsCount)

# Part 2: Count ranges which overlap at all

intersectionsCount: int = 0
for line in lines:
    # another one liner coming up?
    r: list[int] = lineToList(line)
    # you better believe it
    intersectionsCount += ( (r[0] - r[3]) * (r[1] - r[2]) <= 0 )
